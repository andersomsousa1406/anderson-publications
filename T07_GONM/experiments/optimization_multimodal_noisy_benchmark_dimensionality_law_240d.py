...
OUT_DIR = ROOT / "GONM" / "results" / "dimensionality_law_240d"
SEEDS = [100 + 37 * i for i in range(4)]
ACKLEY_DIMS = [24, 240]
BUDGET = 1040
PHASE1_RATIOS = [0.52, 0.80, 0.90]
NOISE_STD = 0.18
...
def run_gonm_nd(surface: bench.Surface, seed: int, budget: int, noise_std: float, phase1_ratio: float) -> bench.RunResult:
    ...
    for center in candidate_centers:
        if oracle.evals >= budget:
            break
        ref_x, ref_true, ref_noisy = cpp_local_refine(oracle, center, budget=budget, max_iters=6)
...
def main() -> None:
    ...
    summary_lines = ["# GONM Dimensionality Law 240D", "", "Budget fixo: `1040` | seeds: `4`", ""]
    results = {"phase1_sweep_240d": [], "scaling": []}

    surface_240 = make_ackley_surface(240)
    summary_lines.extend(["## 240D Phase-1 Sweep", ""])
    for ratio in PHASE1_RATIOS:
        rows = []
        for seed in SEEDS:
            result = run_gonm_nd(surface_240, seed=seed, budget=BUDGET, noise_std=NOISE_STD, phase1_ratio=ratio)
            probe = make_initial_probe(surface_240, seed)
            initial_true = surface_240.func(probe)
            rows.append({
                **asdict(result),
                "phase1_ratio": ratio,
                "initial_true_value": initial_true,
                "residual_ratio": residual_ratio(result.best_true_value, initial_true, surface_240.global_min_value),
            })
        stats = summarize(rows)
        results["phase1_sweep_240d"].append({"phase1_ratio": ratio, "summary": stats, "runs": rows})
    ...
    summary_lines.extend(["## Scaling Check: 24D vs 240D", ""])
    methods = {
        ...
        "gonm_p90": lambda surface, seed, budget, noise_std: run_gonm_nd(surface, seed, budget, noise_std, 0.90),
    }
