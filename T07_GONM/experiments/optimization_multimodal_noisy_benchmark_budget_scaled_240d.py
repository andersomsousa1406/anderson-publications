...
OUT_DIR = ROOT / "GONM" / "results" / "budget_scaled_240d"
SEEDS = [100, 137, 174]
DIM = 240
BUDGETS = [1040, 4160, 16640]
...
def eval_method(surface: bench.Surface, method_name: str, budget: int, phase1_ratio: float | None = None) -> dict:
    rows = []
    for seed in SEEDS:
...
def main() -> None:
    ...
    summary_lines = ["# Budget-Scaled 240D", "", f"Dimensao: `{DIM}` | seeds: `{len(SEEDS)}`", ""]
    results = []
    ...
    for budget in BUDGETS:
        ...
        for method_name, phase1_ratio in [("sim_anneal", None), ("csd", None), ("gonm_p80", 0.80), ("gonm_p95", 0.95)]:
            row = eval_method(surface, method_name, budget, phase1_ratio)
            results.append({
                "budget": budget,
                "method": method_name,
                **row,
            ...
            summary_lines.append(
                f"- `{method_name}`: best_true_mean=`{stats['best_true_mean']:.4e}`, residual_ratio_mean=`{stats['residual_ratio_mean']:.4e}`, wall_coeff=`{row['wall_coeff']:.4e}`, residual_wall_coeff=`{row['residual_wall_coeff']:.4e}`"
            )
        summary_lines.append("")
