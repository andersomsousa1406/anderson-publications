...
OUT_DIR = ROOT / "GONM" / "results" / "dimensionality_law_1000d"
SEEDS = [100, 137, 174]
ACKLEY_DIMS = [240, 1000]
BUDGET = 1040
PHASE1_RATIOS = [0.80, 0.95]
NOISE_STD = 0.18
...
def cpp_local_refine(oracle: bench.NoisyOracle, x0: np.ndarray, budget: int, max_iters: int) -> tuple[np.ndarray, float, float]:
...
def run_gonm_nd(surface: bench.Surface, seed: int, budget: int, noise_std: float, phase1_ratio: float) -> bench.RunResult:
    ...
    for center in candidate_centers:
        if oracle.evals >= budget:
            break
        ref_x, ref_true, ref_noisy = cpp_local_refine(oracle, center, budget=budget, max_iters=3)
...
def main() -> None:
    ...
    summary_lines = ["# GONM Dimensionality Law 1000D", "", "Budget fixo: `1040` | seeds: `3`", ""]
    results = {"phase1_sweep_1000d": [], "scaling": []}

    surface_1000 = make_ackley_surface(1000)
    summary_lines.extend(["## 1000D Phase-1 Sweep", ""])
    for ratio in PHASE1_RATIOS:
        rows = []
        for seed in SEEDS:
            result = run_gonm_nd(surface_1000, seed=seed, budget=BUDGET, noise_std=NOISE_STD, phase1_ratio=ratio)
            probe = make_initial_probe(surface_1000, seed)
            initial_true = surface_1000.func(probe)
            rows.append({
                **asdict(result),
                "phase1_ratio": ratio,
                "initial_true_value": initial_true,
                "residual_ratio": residual_ratio(result.best_true_value, initial_true, surface_1000.global_min_value),
            })
        stats = summarize(rows)
        results["phase1_sweep_1000d"].append({"phase1_ratio": ratio, "summary": stats, "runs": rows})
        summary_lines.append(
            f"- `gonm_p{int(round(100 * ratio))}`: best_true_mean=`{stats['best_true_mean']:.4e}`, dist_mean=`{stats['dist_mean']:.4e}`, residual_ratio_mean=`{stats['residual_ratio_mean']:.4e}`"
        )
    summary_lines.append("")

    summary_lines.extend(["## Scaling Check: 240D vs 1000D", ""])
    methods = {
        "sim_anneal": bench.run_simulated_annealing,
        "csd": bench.run_csd,
        "gonm_p80": lambda surface, seed, budget, noise_std: run_gonm_nd(surface, seed, budget, noise_std, 0.80),
        "gonm_p95": lambda surface, seed, budget, noise_std: run_gonm_nd(surface, seed, budget, noise_std, 0.95),
    }
    for dim in ACKLEY_DIMS:
        ...
        for method_name, method_fn in methods.items():
            ...
            summary_lines.append(
                f"- `{method_name}`: best_true_mean=`{stats['best_true_mean']:.4e}`, residual_ratio_mean=`{stats['residual_ratio_mean']:.4e}`, wall_coeff=`{wall_coeff:.4e}`, residual_wall_coeff=`{residual_wall_coeff:.4e}`"
            )
        summary_lines.append("")
