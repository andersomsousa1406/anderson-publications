...
OUT_DIR = ROOT / "GONM" / "results" / "dimensional_sweep_10_to_100"
SEEDS = [100, 137, 174, 211]
DIMS = list(range(10, 101, 10))
BUDGET = 1040
NOISE_STD = 0.18
...
def run_gonm_nd(surface: bench.Surface, seed: int, budget: int, noise_std: float, phase1_ratio: float) -> bench.RunResult:
    ...
    for center in candidate_centers:
        if oracle.evals >= budget:
            break
        ref_x, ref_true, ref_noisy = cpp_local_refine(oracle, center, budget=budget, max_iters=8)
...
def eval_method(surface: bench.Surface, method_name: str, phase1_ratio: float | None = None) -> dict:
    rows = []
    for seed in SEEDS:
        if method_name == "sim_anneal":
            result = bench.run_simulated_annealing(surface, seed=seed, budget=BUDGET, noise_std=NOISE_STD)
        elif method_name == "csd":
            result = bench.run_csd(surface, seed=seed, budget=BUDGET, noise_std=NOISE_STD)
        else:
            result = run_gonm_nd(surface, seed=seed, budget=BUDGET, noise_std=NOISE_STD, phase1_ratio=phase1_ratio)
    ...
    return {
        "summary": stats,
        "wall_coeff": stats["best_true_mean"] / sqrt(surface.dim),
        "residual_wall_coeff": stats["residual_ratio_mean"] / sqrt(surface.dim),
        "runs": rows,
    }
...
def main() -> None:
    ...
    summary_lines = ["# Dimensional Sweep 10D to 100D", "", f"Budget fixo: `{BUDGET}` | seeds: `{len(SEEDS)}`", ""]
    results = []

    for dim in DIMS:
        surface = make_ackley_surface(dim)
        summary_lines.append(f"## dim=`{dim}`")
        for method_name, phase1_ratio in [("sim_anneal", None), ("csd", None), ("gonm_p80", 0.80), ("gonm_p95", 0.95)]:
            row = eval_method(surface, method_name, phase1_ratio)
            results.append({
                "dim": dim,
                "method": method_name,
                **row,
            ...
            summary_lines.append(
                f"- `{method_name}`: best_true_mean=`{stats['best_true_mean']:.4e}`, dist_mean=`{stats['dist_mean']:.4e}`, residual_ratio_mean=`{stats['residual_ratio_mean']:.4e}`, wall_coeff=`{row['wall_coeff']:.4e}`, residual_wall_coeff=`{row['residual_wall_coeff']:.4e}`"
            )
        summary_lines.append("")
